    ...
    const syncDbRoles = async () => {
      try {
        ...
        if (data && data.length > 0) {
          ...
          const newRoles: Role[] = data.map(dbRole => {
            newMap[dbRole.ma_vai_tro] = dbRole.vai_tro_id;
            ...
            const rId = dbRole.ma_vai_tro;

            permissionsObj['ticket:view'] = hasTicketView || rId === 'QUAN_LY' || rId === 'IT_L1' || rId === 'IT_L2' || rId === 'NGUOI_YEU_CAU';
            permissionsObj['ticket:assign'] = hasTicketAssign || rId === 'QUAN_LY' || rId === 'IT_L1' || rId === 'IT_L2';
            permissionsObj['ticket:close'] = hasTicketClose || rId === 'QUAN_LY' || rId === 'IT_L1' || rId === 'IT_L2';
            permissionsObj['ticket:escalate'] = hasTicketEscalate || rId === 'QUAN_LY' || rId === 'IT_L1';
            permissionsObj['reports:view'] = hasReportsView || rId === 'QUAN_LY' || rId === 'IT_L1';
            permissionsObj['sla:manage'] = hasSlaManage || rId === 'QUAN_LY';
            permissionsObj['kb:create'] = hasKbCreate || rId === 'IT_L2' || rId === 'QUAN_LY';
            permissionsObj['admin:manage'] = hasAdminManage || rId === 'QUAN_LY';

            return {
              id: dbRole.ma_vai_tro,
              name: dbRole.ten_vai_tro,
          ...
          setError('Không tìm thấy vai trò nào trên hệ thống.');
        }
      } catch (err: any) {
        console.error('Failed to sync backend role permissions:', err);
        setError('Không thể kết nối đến máy chủ để tải phân quyền.');
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSavePermissions = async () => {
    ...
    try {
      ...
      if (dbRoleId) {
        ...
        alert('Không tìm thấy ID vai trò hợp lệ để lưu.');
      }
    } catch (err: any) {
      ...
      <div className="settings-roles-layout-split">
        {loading ? (
          <div style={{ padding: '40px', textAlign: 'center', color: '#64748B', fontWeight: 600 }}>
            🔄 Đang tải danh sách vai trò phân quyền từ máy chủ...
          </div>
        ) : error ? (
